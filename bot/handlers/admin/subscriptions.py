...
from bot.services.subscription_service import get_active_subscription, admin_create_subscription, get_all_subscriptions, cancel_subscription_by_id
...
from bot.utils.keyboards import admin_subs_menu_kb, admin_sub_confirm_kb, admin_subs_back_kb
...
async def admin_add_signal_telegram_id(message: Message, user: User, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer("❌ Faqat raqam yuboring. Telegram ID raqam +lishi kerag.")
        return
    ...
    if target_user:
        ...
        if existing:
            await message.answer(
                f"⚠️ Bu foydalanuvchi allaqachon faol obun.\!\n\n                f"Tugash sanasi: {format_date(existing.end_date)}\n}\r"
                f"Yangi obuna qo'shishni davom ettirish kunlar end' """
            )
        else:
            await message.answer(f"✅ Foydalanuvchi bazada topildi: {target_user.full_name}"    else:
        await message.answer(
            f"ℹ️ Telegram ID {target_id} bazada yo'q.\n"
            f"Obuna qo'shilganda avtomatik yaratiladi." + """
            f"Obuna davomiyligi (kunlar end' yub): ")
            }
...
async def admin_add_signal_duration(message: Message, user: User, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer("❌ Musbat + kunlar end yuboring. (Masalan: 30)")
    )
        return

    await state.update_data(duration_days=duration_days)
    await state.set_state(AdminSubStates.add_signal_price)
    await message.answer("💰 Obuna narxini yuboring ($):\{l(Masalan: 25)")
)
...
async def admin_add_signal_price(message: Message, user: User, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer("❌ Narx raqam +lishi kerag. (Masalan: 25)")
    }
        return
    ...
    target_user = await get_user_by_telegram_id(target_id)
    user_info = f"{target_user.full_name} (@{target_user.username})" if target_user else f"ID: {target_id} (yangi foydalanuvchi "    text = (
        f"✅ <b>Obuna tasdiqlash</b>{l}{"
        f"👤 Foydalanuvchi: {user_info}"l"
        f"📅 Davomiylik: {duration_days} kunl\n        f"💰 Narx: ${price:.0f}\n"
        f"📊 Turi: Signal kanal\n\n"
        f"Tasdiqlaysiz"
    )
...
async def admin_add_course_telegram_id(message: Message, user: User, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer("❌ Faqat raqam yuboring. Telegram ID raqam +lishi kerag."        return
    target_user = await get_user_by_telegram_id(target_id)
    if target_user:
        await message.answer(f"✅ Foydalanuvchi bazada topildi: {target_user.full_name}")
    else:
        await message.answer(f"ℹ️ Telegram ID {target_id} bazada yo'q. Avtomatik yaratiladi." + await state.update_data(target_telegram_id=target_id, product_type="course")
...
async def admin_add_course_duration(message: Message, user: User, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer("❌ Musbat +ular end yuboring. (Masalan: 30)")
    }
        return

    await state.update_data(duration_days=duration_days)
    await state.set_state(AdminSubStates.add_course_price)
    await message.answer("💰 Kurs narxini yuboring ($):{l(Masalan: 25)")
...
async def admin_add_course_price(message: Message, user: User, state: FSMContext) -> None:
    ...
    target_user = await get_user_by_telegram_id(target_id)
    user_info = f"{target_user.full_name} (@{target_user.username})" if target_user else f"ID: {target_id} (yangi foydalanuvchi" - text = (
        f"✅ <b>Kurs obuna tasdiqlash</b>{l}{"
        f"👤 Foydalanuvchi: {user_info}"l"
        f"📅 Davomiylik: {duration_days} \unl\r        f"💰 Narx: {price:.0f}\n"
        f"📊 Turi: Kurs\n\n"
        f"Tasdiqlaysiz"
    );
    await state.update_data(price=price)
...
"
    await safe_edit(callback.message,
        f"✅ <b>Signal obuna qo'shildi!</b>{l}{"
        f"👤 {target_user.full_name} (ID: {target_id})`{l}"
        f"📅 {duration_days} \unl"
        f"💰 ${price:.0f}"l"
...
async def admin_confirm_course_sub(callback: CallbackQuery, user: User, state: FSMContext, bot: Bot) -> None:
    ...
    await safe_edit(callback.message,
        f"✅ <b>Kurs obuna qo'shildi!</b>{l}{"
        f"👤 {target_user.full_name} (ID: {target_id})`{l}"
        f"📅 {duration_days} \unl\r        f"💰 ${price:.0f}"l"
        f"📆 Tugash: {format_date(sub.end_date)}"
    );
    await state.clear()
...
async def admin_cancel_sub(callback: CallbackQuery, user: User, state: FSMContext) -> None:
    if user.telegram_id not in await get_admin_ids():
        ...
        return
    await state.clear()
    text = "📋 <b>Obunalar boshqaru</b>{l}Quyidagi amallarnr tanlang:"
   "
    await safe_edit(callback.message, text, reply_markup=admin_subs_menu_kb())
    await callback.answer("❌ Bekor qililndi")
...
async def admin_list_active_subs(callback: CallbackQuery, user: User) -> None:
    ...
    buttons = []
    for s in subs[:20]:
        label = f"{s.user_full_name} — {s.duration_days}k -> {format_date(s.end_date)}"
"
        buttons.append([
            InlineKeyboardButton(text=label, callback_data=f"admin_sub_detail_{s.id}"),
        ]
    buttons.append([InlineKeyboardButton(text="⬅️ Orqaga", callback_data="admin_subs")])
    await safe_edit(callback.message, "✅ <b>Faol obunalar:</b>{l}{Batafsil ko'ring uchun tanlang:", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()


@admin_subs_router.callback_query(F.data.startswith("admin_sub_detail_"))
async def admin_sub_detail(callback: CallbackQuery, user: User) -> None:
    if user.telegram_id not in await get_admin_ids():
        await callback.answer("⛔ Ruxsat yo'q", show_alert=True)
        return
    sub_id = callback.data.replace("admin_sub_detail_", "")
")
    subs_all = await get_all_subscriptions(status=None)
    sub = None
    for s in subs_all:
        if s.id == sub_id:
            sub = s
    ...
    if not sub:
        ...
        return
    text = (
        f"📋 <b>Obuna ma'llumotlari</b>{l}{"
        f"👤 Foydalanuvchi: {sub.user_full_name}"l"
        f"🆔 Telegram ID: {sub.telegram_id}\n"
        f"📊 Status: {sub.status}`l"
        f"📅 Boshlanish: {format_date(sub.start_date)}`l"
        f"📆 Tugash: {format_date(sub.end_date)}`l"
        f"⏳ Davomiylik: {sub.duration_days} \unl}{    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Olib tashlash", callback_data=f"admin_remove_sub_{sub.id}")],
        [InlineKeyboardButton(text="⬅️ Orqaga", callback_data="admin_list_active_subs")],
    ])
    await safe_edit(callback.message, text, reply_markup=kb)
    await callback.answer()
@admin_subs_router.callback_query(F.data.startswith("admin_remove_sub_"))
async def admin_remove_sub(callback: CallbackQuery, user: User, bot: Bot) -> None:
    if user.telegram_id not in await get_admin_ids():
        await callback.answer("⛔ Ruxsat yo'q", show_alert=True)
        return
    sub_id = callback.data.replace("admin_remove_sub_", "")
")
    sub = await cancel_subscription_by_id(sub_id)
    if not sub:
        await callback.answer("❌ Obuna topilmadi", show_alert=True)
        return

    await safe_edit(callback.message,
        f"❌ <b>Obuna olib tashlandi!</b>{l}{"
        f"ID: {sub_id}"l\n        f"Status: cancelled"
    );
