...
async def admin_list_active_subs(callback: CallbackQuery, user: User) -> None:
    ...
    buttons = []
    for s in subs[:20]:
        label = f"{s.user_full_name} — {s.duration_days} kun → {format_date(s.end_date)}"
        buttons.append([
...
async def admin_sub_detail(callback: CallbackQuery, user: User) -> None:
    ...
    text = (
        ...
        f"👤 Foydalanuvchi: {sub.user_full_name}\n"
        f"🆔 Telegram ID: {sub.telegram_id}\n"
        f"📊 Status: {sub.status}\n"
        f"📅 Boshlanish: {format_date(sub.start_date)}\n"
        f"📆 Tugash: {format_date(sub.end_date)}\n"
        f"⏳ Davomiylik: {sub.duration_days} kun"
    )
